...
from config.settings import (
	MODEL, BEDROCK_REGION, BEDROCK_TEMPERATURE,
	BEDROCK_TOP_P, BEDROCK_MAX_TOKENS,
...
class BriefService:
	...
	def __init__(self):
		self.chat_model = ChatBedrockConverse(
			model=Constants.BEDROCK_LLM_MODELS[MODEL],
	...
	async def generate_brief(self, research_text: str, objectives: str, system_prompt: str | None = None) -> CampaignBrief:
		...
		async with self.limiter:
			...
			parsed.input_tokens = response["raw"].usage_metadata.get("input_tokens", -1)
			parsed.output_tokens = response["raw"].usage_metadata.get("output_tokens", -1)
			return parsed
